...
const Privacy: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-black h-full overflow-hidden relative font-sans">
      ...
      <main className="flex-1 px-5 pt-6 space-y-8 overflow-y-auto no-scrollbar pb-40">
        ...
        {/* BLOQUE 2: CONFIGURACIÓN IA (Solo selección, sin toggles) */}
        <section className="space-y-4">
            <SectionHeader title="calibración del mentor" icon="psychology" />
            ...
            <div className="liquid-glass rounded-2xl p-4 space-y-3">
                <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">profundidad de respuesta</span>
                <div className="space-y-2">
                    {(['breve', 'normal', 'profunda'] as Depth[]).map(d => (
                        <button 
                            key={d}
                            onClick={() => setProfile((p:any) => ({...p, aiDepth: d}))}
                            className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition-colors lowercase ${profile.aiDepth === d ? 'bg-white/10 text-white' : 'text-neutral-600'}`}
                        >
                            {d}
                        </button>
                    ))}
            ...
            <div className="flex flex-wrap gap-2">
                {goals.map(g => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-full border text-xs font-bold transition-all lowercase ${selectedGoals.includes(g) ? 'bg-white text-black border-white' : 'bg-transparent text-neutral-500 border-white/10'}`}
                    >
                        {g}
                    </button>
                ))}
...
// Updated ToggleRow with Icons for the Unified List
...
// Switch visual component (stateless, controlled by parent)
